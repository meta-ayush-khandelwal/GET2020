...
public class Ques1 {
                        ...
                        if (isValidOperator(opt)) {
                            if (prece(opt) > prece(operator.peek())) {
                                ...
                                while (prece(opt) <= prece(operator.peek())) {
                                    String value =
                                            operation(operand.pop(), operand.pop(), operator.pop());

                                    // for the case where values are not similar
                                    if (value.equalsIgnoreCase("error")) {
                                        System.out
                                                .println("There is an error while performing the operation");
                                        return "Error";
                                    }
                                    operand.push(value);
                                }
    ...
    static boolean isValidOperator(String var) {
        if (var.equalsIgnoreCase("+") || var.equalsIgnoreCase("-") || var.equalsIgnoreCase("*")
                || var.equalsIgnoreCase("/") || var.equalsIgnoreCase("==")
                || var.equalsIgnoreCase("!=") || var.equalsIgnoreCase("<=")
                || var.equalsIgnoreCase(">=") || var.equalsIgnoreCase("&&")
                || var.equalsIgnoreCase("||") || var.equalsIgnoreCase("<")
